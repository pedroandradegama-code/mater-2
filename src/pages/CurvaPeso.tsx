...
import { ArrowLeft, Plus, Trash2, TrendingUp, Scale, Info } from 'lucide-react';
    ...
    <div className="bg-card border border-[var(--card-border-color)] rounded-2xl p-3 shadow-lg text-sm">
      ...
      {ganho != null && (
        <p className="text-primary font-medium">
          Seu ganho: {ganho > 0 ? '+' : ''}{Number(ganho).toFixed(1)} kg
        </p>
      )}
      {min != null && max != null && (
        <p className="text-muted-foreground text-xs mt-0.5">
          Faixa ideal: +{Number(min).toFixed(1)} – +{Number(max).toFixed(1)} kg
        </p>
      )}
    </div>
  );
...
export default function CurvaPeso() {
  ...
  const [pesoPreInput, setPesoPreInput] = useState(
    () => localStorage.getItem('mater_peso_pre') || ''
  );
  const [alturaInput, setAlturaInput] = useState(
    () => localStorage.getItem('mater_altura') || ''
  );
  ...
  const handlePesoPreChange = (v: string) => {
    setPesoPreInput(v);
    localStorage.setItem('mater_peso_pre', v);
  };
  const handleAlturaChange = (v: string) => {
    setAlturaInput(v);
    localStorage.setItem('mater_altura', v);
  };
  ...
  const chartData = useMemo(() => {
    ...
    return faixa.map(({ semana, min, max }) => {
      ...
      const ganho =
        pesoRegistrado != null && pesoPre
          ? +(pesoRegistrado - pesoPre).toFixed(2)
          : null;
  ...
  const ganhoTotal =
    ultimaEntrada && pesoPre
      ? +(ultimaEntrada.peso - pesoPre).toFixed(1)
      : null;
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Acompanhamento baseado nas diretrizes do MS
            </p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-[20px] border border-[var(--card-border-color)] p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <Input
                ...
                placeholder="ex: 62,5"
                value={pesoPreInput}
                onChange={e => handlePesoPreChange(e.target.value)}
          ...
          {imc && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm ${cat.bgClass} border border-current/10`}>
              ...
              <span>
                <strong>IMC: {imc}</strong> — {cat.label} &nbsp;·&nbsp; Ganho recomendado:{' '}
                <strong>{cat.totalMin}–{cat.totalMax} kg</strong>
              </span>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-3 gap-2.5 mb-4">
            ...
            <div className="bg-card rounded-[16px] border border-[var(--card-border-color)] p-3">
              ...
              <p className={`font-display text-2xl font-bold leading-none ${
                statusGanho === 'ideal' ? 'text-green-600' :
                statusGanho === 'abaixo' ? 'text-amber-600' :
                statusGanho === 'acima' ? 'text-red-500' :
                'text-foreground'
              }`}>
                {ganhoTotal != null ? `${ganhoTotal > 0 ? '+' : ''}${ganhoTotal}` : '—'}
              </p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {ganhoTotal != null ? 'kg total' : 'sem dados'}
              </p>
            </div>
            <div className="bg-card rounded-[16px] border border-[var(--card-border-color)] p-3">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="font-display text-base font-semibold text-foreground">
                Traçado de ganho de peso
              </p>
              <p className="text-[11px] text-muted-foreground mt-0.5">
                {imc ? `${cat.label} · faixa IOM/MS` : 'Configure os dados iniciais'}
              </p>
            </div>
            ...
            <div className="space-y-3 pt-1">
              <div>
                <label className="text-xs text-muted-foreground mb-1.5 block">
                  Semana gestacional
                  {semanaAtual && (
                    <span className="text-primary ml-1">(atual: semana {semanaAtual})</span>
                  )}
                </label>
                <Input
                  type="number"
              ...
              {formPeso && pesoPre && formSemana && (
                <PreviewStatus
                  pesoPre={pesoPre}
                  pesoAtual={parseFloat(formPeso)}
                  semana={parseInt(formSemana)}
                  faixa={faixa}
                />
              )}
              <Button
                onClick={() => addPeso.mutate()}
                disabled={addPeso.isPending || !formSemana || !formPeso}
                className="w-full rounded-xl bg-primary text-primary-foreground h-11"
              >
          ...
          <div className="bg-card rounded-[20px] border border-[var(--card-border-color)] overflow-hidden mb-4">
            ...
            <div>
              {[...entradas].reverse().map(entrada => {
                ...
                const status =
                  ganho != null && ponto
                    ? ganho < ponto.min ? 'abaixo' : ganho > ponto.max ? 'acima' : 'ideal'
                    : null;
                const statusDot = { ideal: 'bg-green-400', abaixo: 'bg-amber-400', acima: 'bg-red-400' };
                return (
                  <div
                    key={entrada.id}
                    className="flex items-center gap-3 px-4 py-3 border-b border-[var(--card-border-color)] last:border-0"
                  >
                    ...
                    <button
                      onClick={() => {
                        if (deleteId === entrada.id) {
                          deletePeso.mutate(entrada.id);
                        } else {
                          setDeleteId(entrada.id);
...
function PreviewStatus({
  pesoPre, pesoAtual, semana, faixa,
}: {
  pesoPre: number; pesoAtual: number; semana: number;
  faixa: { semana: number; min: number; max: number }[];
}) {
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-[var(--card-border-color)]">
            ...
            {Object.values(CATEGORIAS).map(c => (
              <div
                ...
                className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm ${
                  c.key === catAtiva
                    ? `${c.bgClass} border border-current/20`
                    : 'bg-muted/30'
                }`}
              >
                <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-3 text-center">
            Fonte: Institute of Medicine (IOM) 2009, adotado pelo Ministério da Saúde. Gestação única a termo.
          </p>
        </div>
      )}
    </div>
